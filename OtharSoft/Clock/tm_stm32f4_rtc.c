...
TM_RTC_Result_t TM_RTC_SetDateTime(TM_RTC_t* data, TM_RTC_Format_t format) {
	...
	if (tmp.year > 99 || tmp.month == 0 || tmp.month > 12 || tmp.date == 0
			|| tmp.date
					> TM_RTC_Months[TM_RTC_LEAP_YEAR(2000 + tmp.year) ? 1 : 0][tmp.month
							- 1] || tmp.hours > 23 || tmp.minutes > 59
			|| tmp.seconds > 59 || tmp.day == 0 || tmp.day > 7) {
		/* Invalid date */
...
void TM_RTC_GetDateTimeFromUnix(TM_RTC_t* data, uint32_t unix) {
	...
	for (data->month = 0; data->month < 12; data->month++) {
		if (TM_RTC_LEAP_YEAR(year)
				&& unix >= (uint32_t) TM_RTC_Months[1][data->month]) {
