...
void initializeGPIOTimer() {
	...
	GPIO_PinAFConfig(GPIOE, GPIO_PinSource2, GPIO_AF_TIM2);
...
void checkDimmers(void) {
	...
	//Check dimmer1
	if (TIM_GetCounter(TIM1) == Dimer1) {
		GPIO_SetBits(GPIOE, GPIO_Pin_2);
	}
	if (TIM_GetCounter(TIM1) == dimer1 + TRIACTIMEON) {
		GPIO_ResetBits(GPIOE, GPIO_Pin_2);
	}

	//Check dimmer2
	if (TIM_GetCounter(TIM1) == Dimer2) {
		GPIO_SetBits(GPIOE, GPIO_Pin_2);
	}
	if (TIM_GetCounter(TIM1) == dimer2 + TRIACTIMEON) {
		GPIO_ResetBits(GPIOE, GPIO_Pin_2);
	}

	//Check dimmer3
	if (TIM_GetCounter(TIM1) == Dimer3) {
		GPIO_SetBits(GPIOE, GPIO_Pin_2);
	}
	if (TIM_GetCounter(TIM1) == dimer3 + TRIACTIMEON) {
		GPIO_ResetBits(GPIOE, GPIO_Pin_2);
	}

	//Check dimmer4
	if (TIM_GetCounter(TIM1) == Dimer4) {
		GPIO_SetBits(GPIOE, GPIO_Pin_2);
	}
	if (TIM_GetCounter(TIM1) == dimer4 + TRIACTIMEON) {
		GPIO_ResetBits(GPIOE, GPIO_Pin_2);
	}
